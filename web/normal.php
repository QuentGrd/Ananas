...
if (isset($_POST['pseudo']) && isset($_POST['score'])){
	insertScore($_POST['score'], $_POST['pseudo'], 3);
}
if (!file_exists("res/lb/normal.scr")){
	createFile();
}
else{
	if ($file = fopen('res/lb/normal.scr', 'r')){
		$str = fgets($file);
		$tab = unserialize($str);
		fclose($file);
	}
...
<html>
	<head>
		<title>Urban Life Simulator - Normal</title>
		<meta charset="utf-8">
	...
	<body>
		...
		<section>
			<h2>Classement du niveau normal</h2>
